...
function loadModel(loader) {
    loader.load("../3d_models/wooden_writing_desk/scene.gltf", function (gltf) {
        ...
        console.error("Loading 3d model: ", error);
    });
}
...
function setupEventListeners() {
    ...
    container.addEventListener("click", (event) => {
        if (isChairMoved) return;
    
        updateMousePosition(event);
        const clickedObject = getIntersectedObject();
    
        if (isTargetObject(clickedObject)) {
            chair = clickedObject;
            ...
            if(isAuthenticated()){
                ...
                alert("To use the functionality, please register or log in to your account.");
                setTimeout(1000);
                showAuthModal();
            }
        }
    ...
    container.addEventListener("mousemove", (event) => {
        updateMousePosition(event);
        const hoveredObject = getIntersectedObject();
    
        container.style.cursor = isTargetObject(hoveredObject) ? "pointer" : "default";
    });
}

//Функції для обробників
...
function isTargetObject(object) {
    return object && (object.parent.name.includes("Chair") || object.parent.name.includes("Table"));
}
